...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
  ...
  public void train(String sourceText) {
    ...
    for (int i = 1; i < list.length; i++) {
      appendListWord(prevWord, list[i]);
      prevWord = list[i];
    }
    prevWord = list[list.length - 1];
    appendListWord(prevWord, starter);
  }
  ...
  public String generateText(int numWords) {
    ...
    if (wordList.size() == 0 || numWords <= 0) {
      ...
      for (int i = 0; i < numWords - 1; i++) {
        String addWord = getNode(currWord).getRandomNextWord(rnGenerator);
        output += addWord + " ";
        currWord = addWord;
      }
      return output;
  ...
  private boolean isNode(String word) {
  ...
  private void appendListWord(String prevWord, String newWord) {
    if (!isNode(prevWord)) {
      ListNode newListNode = new ListNode(prevWord);
      wordList.add(newListNode);
      newListNode.addNextWord(newWord);
    } else {
      getNode(prevWord).addNextWord(newWord);
    }
  }
  /**
