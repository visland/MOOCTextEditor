...
public abstract class Document {
  ...
  protected int countSyllables(String word) {
    ...
    boolean lastCharIsSyllable = false;
    int count = 0;
    for (int i = 0; i < word.length(); i++) {
      if ((word.charAt(i) == 'e' || word.charAt(i) == 'a' || word.charAt(i) == 'i' || word.charAt(i) == 'o'
          || word.charAt(i) == 'u' || word.charAt(i) == 'y' || word.charAt(i) == 'E' || word.charAt(i) == 'A'
          || word.charAt(i) == 'I' || word.charAt(i) == 'O' || word.charAt(i) == 'U' || word.charAt(i) == 'Y')
          && !lastCharIsSyllable) {
        if (word.charAt(i) == 'e' && i == word.length() - 1 && !hasSyllable) {
          count++;
        } else if (!(word.charAt(i) == 'e' && i == word.length() - 1)) {
          count++;
          lastCharIsSyllable = true;
          hasSyllable = true;
        }
      } else if (word.charAt(i) != 'e' && word.charAt(i) != 'a' && word.charAt(i) != 'i' && word.charAt(i) != 'o'
          && word.charAt(i) != 'u' && word.charAt(i) != 'y' && word.charAt(i) != 'E' && word.charAt(i) != 'A'
          && word.charAt(i) != 'I' && word.charAt(i) != 'O' && word.charAt(i) != 'U' && word.charAt(i) != 'Y') {
  ...
  // For the sake of the precision, Java type will always convert to the 
  // more precise one... (更高精的类型)
  // int / int = int
  // int / float = float
  // float / int = float
  // float / float = float
